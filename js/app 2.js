/**
 * Hauptmodul und Orchestrierung der Anwendung
 * ============================================
 *
 * Aufgabe:
 * Verbindet Spielzustand, Fachmodelle, Rendering und die spezialisierten
 * UI-Controller. Große Teilbereiche werden schrittweise in eigene Module
 * ausgelagert; `app.js` bleibt die zentrale Verdrahtungsstelle.
 *
 * Abhängigkeiten:
 * - Alle Projektmodule aus diesem Verzeichnis sowie Browser-DOM-APIs.
 * - Persistenz erfolgt über `browser-storage.js`; Datenmodelle über
 *   `card-model.js` und `game-state-schema.js`.
 *
 * Liefert an:
 * - Den vollständigen Browser-Bootstrap und die explizite Handler-Map für
 *   delegierte `data-ui-*` Ereignisse. Das Modul exportiert keine öffentliche API.
 */

import {
  useDemoData, appVersion, cardKinds, characterRoles, cardLocations, encounterStatuses,
  effectDurationModes, effectVisibilityModes, itemExecutableEffectTypes, itemExecutableEffectLabels,
  availableConditions, importSecurityLimits, appOperatingMode, appStorageKey, appChannelName,
  demoCardsAutoloadStorageKey, demoEncounterName, mirielBoardAutomationDefaultsVersion
} from "./config.js";
import { createUniqueId } from "./utils.js";
import {
  accessRoles, cardPublicProfiles, cardFieldVisibility, gameActionPermissions, createAccessContext,
  getSafeCardAccessPolicy, isCardControlledByContext, canViewCardField, canEditCardField,
  canPerformAction, dmAccessContext, publicDisplayAccessContext
} from "./access.js";
import { createInitialGameState, createInitialUiState } from "./state.js";
import { gameStateSchema } from "./game-state-schema.js";
import { srd51ContentMeta, srd521ContentMeta } from "./content-metadata.js";
import { srd51SpellLibrary } from "./srd-spell-library.js";
import { srd521SpellLibrary } from "./srd-5.2.1-spell-library.js";
import { createDemoCards, isKnownDemoCardData } from "./demo-data.js";
import {
  inventoryCategoryLabels,
  inventoryCardTemplates,
  getSafeEffectVisibility,
  getSafeOptionalString,
  getSafeNonNegativeInteger,
  createEmptySpellSlots,
  createDefaultSpellcasting,
  parseSpellSaveDcText,
  getSafeSpellLevel,
  createSpellObject,
  createSpellId,
  ...
  groupSpellsByLevel,
  getSafeUsageReset,
  inferUsageResetFromText,
  inferUsageMaxFromText,
  getSafeCardActionType,
  createCardAction,
  clampNumber,
  splitDetailEntries,
  getDetailEntryTitle,
  getDetailEntryBody,
  getSafeCardTraitCategory,
  createCardTrait,
  normalizeInventoryImagePath,
  createInventoryCardFromTemplate,
  getSafeInventoryCategory,
  createInventoryCard,
  createInventoryListItem,
  parseCurrencyFromInventoryText,
  splitInventoryEntries,
  getInventoryEntryCountAndName,
...
import { registerDelegatedUiEvents } from "./ui-events.js";
import { createRenderLifecycle } from "./render-lifecycle.js";
import { createEncounterNavigation } from "./encounter-navigation.js";
...
const {
  ...
  getNextTurnPlan,
  getPreviousTurnPlan
} = createEncounterNavigation({ gameState, getHandCards, getInitiativeCards });

// Browserbezogener Render-Lebenszyklus: hält Scrollpositionen stabil.
const {
...
// Controller kapseln die reine DOM-Navigation der beiden großen Drawer.
const atelierController = createAtelierController({
  closeFloatingDetails: () => closeFloatingDetailsExcept(null),
  lockPageScroll: () => lockTabletPageScroll(),
  schedulePageScrollUnlock: () => scheduleTabletPageScrollLockSync()
});

const cardForgeController = createCardForgeController({
  closeFloatingDetails: () => closeFloatingDetailsExcept(null),
  lockPageScroll: () => lockTabletPageScroll(),
  schedulePageScrollUnlock: () => scheduleTabletPageScrollLockSync(),
  canClose: () => confirmCloseCardForgeWithOpenSpellEditor(),
  beforeClose: () => {
  ...
  getScrollSnapshot: () => getViewportScrollSnapshot(),
  restoreScrollSnapshot: snapshot => restoreViewportScrollSnapshot(snapshot),
  onTabSelected: tabName => {
    const prefix = getVisibleForgePrefix();
    if (tabName === "actions") renderForgeActionManager(prefix);
    if (tabName === "traits") renderForgeTraitManager(prefix);
    if (tabName === "spells") renderVisibleForgeSpellManager();
    if (tabName === "notes") renderForgeNoteManager(prefix);
    if (tabName === "inventory") renderForgeInventoryManager(prefix);
  }
});
...
let lastRenderedMirielBoardContentMode = null;
...
function renderCardsPreservingViewport() {
    renderPreservingViewport(renderCards);
}
...
function nextTurn() {
    const turnPlan = getNextTurnPlan();
    const { handCards, initiativeCards, targetIndex, startsNewRound } = turnPlan;
...
function previousTurn() {
    const turnPlan = getPreviousTurnPlan();
    const { initiativeCards, targetIndex, crossesRoundBoundary } = turnPlan;
...
function setupClickAwayBehavior() {
    registerDrawerAndClickAwayEvents({
        ...
        hasOpenDetailInventoryEditor: function() {
            return activeDetailInventoryCardEditor !== null || activeDetailInventoryListEditor !== null;
        },
        consumeDetailInventoryClickAwaySuppression: function() {
        ...
        closeHeaderHelp: function(targetElement) {
            const headerHelpElement = document.querySelector(".app-header-help");
            if (
                headerHelpElement instanceof HTMLDetailsElement
                && headerHelpElement.open === true
                && targetElement.closest(".app-header-help") === null
            ) {
                headerHelpElement.open = false;
            }
        },
        handleOpenForgeEditorClickAway: function(targetElement) {
            const drawerElement = getCardForgeDrawerElement();
            if (
                drawerElement === null
                || drawerElement.classList.contains("card-forge-drawer-open") !== true
                || drawerElement.contains(targetElement) !== true
            ) {
                return;
            }
            ...
            if (
                ...
                || clickedForgeSpellAction === true
                || clickedForgeActionAction === true
                || clickedForgeTraitAction === true
                || clickedForgeInventoryAction === true
            ) {
...
function createGameStateExportData() {
    return {
        formatName: gameStateSchema.formatName,
        schemaVersion: gameStateSchema.supportedSchemaVersion,
        exportedAt: new Date().toISOString(),
        metadata: {
...
async function handleGameStateImportFileChange(event) {
    ...
    try {
        ...
        const preparationResult = gameStateSchema.parseAndPrepareImport(fileText, importSecurityLimits);
...
function importGameStateData(gameStateData) {
    const validationResult = gameStateSchema.validateGameStateData(gameStateData, importSecurityLimits);
    ...
    const normalizedGameState = gameStateSchema.normalizeGameStateData(gameStateData);
...
function validateGameStateImport(importData) {
    const validationResult = gameStateSchema.validateEnvelope(importData, importSecurityLimits);
...
function getForgeSrdLibrary(sourceKey) {
    if (sourceKey === srd521ContentMeta.key) {
        return {
            meta: srd521ContentMeta,
            spells: srd521SpellLibrary
        };
    }

    return {
        meta: srd51ContentMeta,
...
function getCardForgeDrawerElement() {
    return cardForgeController.getElement();
}

function openCardForgeDrawer() {
    cardForgeController.open();
}

function closeCardForgeDrawer() {
    return cardForgeController.close();
}
...
function getDmActionDrawerElement() {
    return atelierController.getElement();
}

function openDmActionDrawer(panelName = "toolbox") {
    atelierController.open(panelName);
}

function closeDmActionDrawer() {
    atelierController.close();
}

function setAtelierPanel(panelName) {
    atelierController.setPanel(panelName);
}

function closeAtelierHelp() {
    atelierController.closeHelp();
}

function toggleAtelierHelp() {
    atelierController.toggleHelp();
}
...
function setForgeTab(tabName) {
    cardForgeController.setTab(tabName);
}
...
const uiActionHandlers = Object.freeze({
  ...
  addConditionToSelectedCards,
  addCustomEffectToSelectedCards,
  addForgeAction,
  addForgeInventoryListItem,
  addForgeNoteSection,
  addForgeSpell,
  addForgeSrdSpell,
  addForgeTrait,
  applyDamageToSelectedCards,
  applyHealingToSelectedCards,
  applyInitiativeToSelectedCards,
  applyTempHpToSelectedCards,
  cancelDetailInventoryEditor,
  cancelForgeActionEditor,
  cancelForgeInventoryEditor,
  cancelForgeSpellEditor,
  cancelForgeTraitEditor,
  clearAllTempHp,
  clearCardSelection,
  clearConditionsFromHandCards,
  clearDeckSelection,
  clearMirielBoardManualImage,
  clearMirielBoardManualText,
  clearRecentUndoOffer,
  clearSavedBrowserState,
  closeCardForgeDrawer,
  closeDeckImportPreview,
  closeDmActionDrawer,
  closeForgeSrdSpellLibrary,
  closeTabletConsole,
  commitForgeInventoryIfEditing,
  confirmDeckImportAppend,
  confirmDeckImportReplace,
  copyCardToDeck,
  deleteAllCards,
  deleteForgeAction,
  deleteForgeInventoryCard,
  deleteForgeInventoryListItem,
  deleteForgeNoteSection,
  deleteForgeSpell,
  deleteForgeTrait,
  endCombat,
  exportEncounterChronicleJson,
  exportEncounterReportHtml,
  exportGameState,
  focusActiveCard,
  focusAdjacentHandCard,
  focusPublicCard,
  handleForgeNoteTitleChange,
  longRest,
  moveAllHandCardsToDeck,
  moveCardToDeck,
  moveCardToHand,
  moveCardToTrash,
  moveHandCardsOfTypeToDeck,
  moveSelectedDeckCardsToHand,
  nextTurn,
  openAppView,
  openDeckLibraryFromToolkit,
  openDetailInventoryCardEditor,
  openDetailInventoryListEditor,
  openDmActionDrawer,
  openEditCardForm,
  openFocusedCardForgeFromToolkit,
  openForgeActionEditor,
  openForgeInventoryEditor,
  openForgeSpellEditor,
  openForgeSrdSpellLibrary,
  openForgeTraitEditor,
  openNewCardForge,
  openNewCardForgeFromToolkit,
  openTabletConsole,
  permanentlyDeleteCard,
  previousTurn,
  reloadDemoCards,
  removeConditionFromSelectedCards,
  removeDemoCards,
  removeInventoryCardFromDetails,
  removeInventoryListItem,
  renameEncounter,
  resetCombatTurnCounter,
  resetPublicFocusToActiveCard,
  restoreCardFromTrash,
  rollInitiativeForHandEnemies,
  rollInitiativeForSelectedCards,
  saveCurrentStateManually,
  saveDetailInventoryCardEditor,
  saveDetailInventoryListEditor,
  saveForgeAction,
  saveForgeInventoryCard,
  saveForgeInventoryListItem,
  saveForgeSpell,
  saveForgeTrait,
  scrollCardRow,
  scrollPlayerSide,
  ...
  setFocusedDeckCard,
  setForgeTab,
  setMirielBoardDurationMode,
  setMirielBoardManualText,
  setMirielBoardManualTextPosition,
  setMirielBoardManualTextSize,
  setMirielBoardPersistentMode,
  shiftInventoryStage,
  showDeckCardFromFocus,
  startEncounter,
  syncForgeNotesTextarea,
  toggleArcaneSelect,
  toggleAtelierHelp,
  toggleCardSelection,
  toggleCardTurnOrder,
  toggleCombatLogEventDetails,
  toggleCombatLogInlinePanel,
  toggleForgeSpellPrepared,
  toggleMirielBoardAutoTurn,
  toggleMirielBoardNewRoundCall,
  toggleSpellDetail,
  toggleSpellPrepared,
  toggleSpellSlot,
  transferInventoryCard,
  triggerGameStateImport,
  undoGameEvent,
  undoRecentGameEvent,
  updateCustomEffectDurationFields,
